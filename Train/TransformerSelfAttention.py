...
import torch
import math
from Train.RotaryPositionalEmbedding import RotaryPositionalEmbedding
from Train.RotaryPositionalEmbedding import apply_rotary_pos_emb
...
class TransformerSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        if FLASH_ATTENTION_AVAILABLE:
            # Ensure inputs are in fp16 or bf16
            dtype = torch.float16 if query_layer.dtype == torch.float32 else query_layer.dtype
            query_layer = query_layer.to(dtype)
            key_layer = key_layer.to(dtype)
            value_layer = value_layer.to(dtype)
